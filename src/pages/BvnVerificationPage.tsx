...
import { useProfile, type BvnVerificationResult } from "@/hooks/useProfile";
...
const BvnVerificationPage = () => {
  const { verifyBvn } = useProfile();
  const [bvn, setBvn] = useState("");
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<BvnVerificationResult | null>(null);
  ...
  useEffect(() => {
    api
      .get<PricingItem[]>("/pricing?service_type=bvn_verification")
      .then((rows) => {
        const basic =
          rows.find(
            (row) =>
  ...
  const handleVerify = async () => {
    ...
    setLoading(true);
    try {
      const data = await verifyBvn(clean);
      setResult(data);
      toast({
        title: "BVN verified",
      ...
      toast({
        ...
        description:
          (err as Error).message || "Could not verify BVN right now.",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      doc.setFontSize(18);
      doc.text("BVN Verification Result", 14, currentY);
      currentY += 8;
      ...
      const rows: Array<[string, string]> = [
        ...
        ["Phone", result.phone || "-"],
        ["Match Result", result.match_result || "-"],
        ["Charge", formatCurrency(result.charged_amount)],
        ["Balance After", formatCurrency(result.new_balance)],
      ];
      ...
      doc.save(`bvn-verification-${result.bvn}.pdf`);
    } catch (error) {
      toast({
        title: "PDF download failed",
        description: error instanceof Error ? error.message : "Could not generate BVN PDF right now.",
        variant: "destructive",
      });
    } finally {
          ...
          <section className="rounded-2xl border border-border/50 bg-card p-5 space-y-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Enter an 11-digit BVN to run a lookup through the active identity provider.
              </p>
            </div>
            ...
            <section className="rounded-2xl border border-border/50 bg-card p-5 space-y-3">
              ...
              <div className="grid gap-3 text-sm">
                ...
                <div className="flex items-center justify-between rounded-xl bg-secondary/50 px-3 py-2 border border-border/50">
                  ...
                  <span className="font-medium text-foreground">
                    {formatCurrency(result.charged_amount)}
                  </span>
                </div>
                <div className="flex items-center justify-between rounded-xl bg-secondary/50 px-3 py-2 border border-border/50">
                  <span className="text-muted-foreground">Balance After</span>
                  <span className="font-medium text-foreground">
                    {formatCurrency(result.new_balance)}
                  </span>
                </div>
              </div>
...
function formatCurrency(value: number | null | undefined): string {
  ...
  return `${siteConfig.currency.symbol}${value.toLocaleString()}`;
}
